...
import static com.grpc.test.grpcstarterclienttest.loadbalance.CustomRoundRobinLoadBalancerFactory.SERVER_NAME;
import static com.grpc.test.grpcstarterclienttest.loadbalance.CustomRoundRobinLoadBalancerFactory.TENANT_ID;
        ...
        public void run() {
            ...
            try {
                ...
                if (!CollectionUtils.isEmpty(newServiceInstanceList)) {
                    ...
                    //add by junqiang.xiao 20180530 增加传入tenant 信息
                    Attributes attributesTenant;
                    String tenantID = "-1";
                    //end add
                    for (ServiceInstance serviceInstance : serviceInstanceList) {
                        ...
                        if (metadata.get("gRPC") != null) {
                            ...
                            if (metadata.containsKey("tenantID")) {
                                ...
                                log.info("Found gRPC tenantID {}", tenantID);
                            }
                            attributesTenant = Attributes.newBuilder().set(TENANT_ID, tenantID).set(SERVER_NAME,serviceInstance.getServiceId()).build();
                            //end add
                            ...
                            EquivalentAddressGroup addressGroup = new EquivalentAddressGroup(new InetSocketAddress(serviceInstance.getHost(), port), attributesTenant);
                            //end modifby
        ...
        if (serviceInstanceList.size() == newServiceInstanceList.size()) {
            for (ServiceInstance serviceInstance : serviceInstanceList) {
                ...
                for (ServiceInstance newServiceInstance : newServiceInstanceList) {
                    //modify by junqiang.xiao 20180530 增加实例判断 tenantID 是否一致
                    //if (newServiceInstance.getHost().equals(serviceInstance.getHost()) && newServiceInstance.getPort() == serviceInstance.getPort()) {
                    if (newServiceInstance.getHost().equals(serviceInstance.getHost()) && newServiceInstance.getPort() == serviceInstance.getPort()
                            && newServiceInstance.getMetadata().get("tenantID") == serviceInstance.getMetadata().get("tenantID")) {
                        //end modify
                        isSame = true;
                        break;
                    }
